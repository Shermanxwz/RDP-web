...
func TestURI(t *testing.T) {
	...
	if strings.Contains(strings.ToLower(got), "password") {
		t.Fatal("URI must never contain a password")
	}
}
...
func TestFileIsUTF16AndSafeDefaults(t *testing.T) {
	...
	for _, want := range []string{"full address:s:[2001:db8::1]:3389", "username:s:管理员", "redirectclipboard:i:1", "redirectprinters:i:0", "drivestoredirect:s:", "use multimon:i:1"} {
		if !strings.Contains(text, want) {
			t.Fatalf("file missing %q: %q", want, text)
		}
	}
	if strings.Contains(strings.ToLower(text), "password") {
		t.Fatal("file must never contain a password")
	}
}
